...
if ($databaseExists -eq 0) {
    ...
    if ($requestTableExists -eq 0) {
        ...
        $requestorIncludedApplied = Invoke-MySqlScalarInt `
            "SELECT COUNT(*) FROM gate_pass_system.tbl_schema_versions WHERE version_no='023';"

        if ($requestorIncludedApplied -eq 0) {
        ...
        $passDateApplied = Invoke-MySqlScalarInt `
            "SELECT COUNT(*) FROM gate_pass_system.tbl_schema_versions WHERE version_no='025';"

        if ($passDateApplied -eq 0) {
