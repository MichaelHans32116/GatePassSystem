...
| Authentication | Login with Employee ID + password → JWT access token |
...
**Why can't I log in?**
Check that your Employee ID and password are correct. If the account is new or was
reset, you may need to change the password on first login. If the problem persists,
